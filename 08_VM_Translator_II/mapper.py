...
def branching_add_label(label: str):
    return f'({label})\n'

def branching_add_goto(label: str):
    return (
        f'@{label}\n'
        f'0; JMP\n'
    )

def branching_add_if_goto(label: str):
    return (
        f'{decrement_stack_pointer()}'
...
def push_segment_to_stack(segment: str):
    return (
        f'@{segment}\n'
        f'D=M\n'
        f'{_push_d_to_stack()}'
...
def routine_call_f(fname, n_args, identifier):
    
    """
    Implements function call by doing:
    ...
    (return address)         // declares a label for the return-address
    """

    return (
        # Push returnAddress
        f'@{_get_function_label(fname)}$ret.{identifier}\n'
        f'D=A\n'
        f'@SP\n'
        f'A=M\n'
        f'M=D\n'
        ...
        # Save State
        f'{push_segment_to_stack("LCL")}'
        f'{increment_stack_pointer()}'
        f'{push_segment_to_stack("ARG")}'
        f'{increment_stack_pointer()}'
        f'{push_segment_to_stack("THIS")}'
        f'{increment_stack_pointer()}'
        f'{push_segment_to_stack("THAT")}'
        f'{increment_stack_pointer()}'
...
def _get_function_label(fname):
    """
    Generates a label for a function.
    
    NOTE: The implicit rule is that function names must be unique
    """

    return f'{fname}'

def routine_function_execute(fname, n_args):
    """Translates function f k."""

    s = f'({_get_function_label(fname)})\n'
...
def routine_function_return():
    
    return (
        # Set temp variable `frame`   FRAME = LCL
        f'@LCL\n'
        f'D=M\n'
        f'@frame\n'
        f'M=D\n'

        # Set temp variable `ret`  RET = *(FRAME-5)
        f'@5\n'
        f'D=A\n'
        f'@frame\n'
        f'A=M-D\n'
...
def translate_line(line: str, line_number: int):
    ...
    if line.startswith('push constant'):
        ...
        return branching_add_label(label)

    elif line.startswith('goto'):
        label = line.replace('goto', '').strip()
        return branching_add_goto(label)

    elif line.startswith('if-goto'):
        label = line.replace('if-goto', '').strip()
        return branching_add_if_goto(label)

    elif line.startswith('call'):
        fname, n_args = line.replace('call', '').strip().split(' ')
        return routine_call_f(fname, n_args, line_number)

    elif line.startswith('function'):
        fname, n_args = line.replace('function', '').strip().split(' ')
        return routine_function_execute(fname, n_args)
