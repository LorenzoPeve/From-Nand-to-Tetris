class Reader():
    ...
    def remove_whitespace(self, data: list[str]) -> list:
        return [line.strip() for line in data]

    def remove_empty_lines(self, data: list[str]) -> list:
        return [line for line in data if len(line) > 1]

    def remove_comment_lines(self, data: list[str]) -> list:
        return [line for line in data if not line.startswith('//')]

    def remove_inline_comments(self, data: list[str]) -> None:
